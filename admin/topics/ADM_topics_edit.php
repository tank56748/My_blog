	...
	<main>
		...
		<div class="posts">
			...
			<div class="add-post">
				<form method="post" action="?page=ADM_topics_edit&id=<?=$_GET['id']?>">
					<div class="form-item err">
						<? include $_SERVER['DOCUMENT_ROOT'] . "/app/include/error_info.php"?>
					</div>
					<input value="<?=$id;?>" name="id" id="h-post" type="hidden">
					<div class="form-item">
						<label for="h-post">Имя категории</label>
						<input value="<?=$name;?>" name="name" id="h-post" type="text" class="reg-input" placeholder="Имя категории...">
					</div>
					<div class="form-item">
					...
					<div class="form-item">
						<button name="button-topic-update" type="submit" class="submit-btn1 add">Обновить категорию</button>
					</div>
				</form>
			</div>
		</div>
