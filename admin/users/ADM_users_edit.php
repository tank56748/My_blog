...
<main>
	...
	<div class="posts">
		...
		<div class="add-post">
			<form method="post" action="?page=ADM_users_edit&id_user=<?=$_GET['id_user']?>">
				<div class="form-item err">
					<? include $_SERVER['DOCUMENT_ROOT'] . "/app/include/error_info.php"?>
				</div>
				<div class="form-item">
					<label for="1">Логин</label>
					<input id="1" name="login" value="<?=$username?>" maxlength="50" type="text" class="reg-input" placeholder="Введите ваш логин...">
				</div>
				<div class="form-item">
				...
				<div class="form-item">
					<div style="margin-bottom:7px;">IP: <?=$ip?></div>
					<div style="margin-bottom:7px;">Страна: <?=$country_index[$country]?></div>
					<div>Город: <?=$city?></div>
				</div>
				<div class="form-item">
					<label for="3">Пароль(оставьте поле пустым если не хотите менять пароль)</label>
					<input id="3" name="password" minlength="5" maxlength="50" type="password" class="reg-input" placeholder="Введите ваш пароль...">
				</div>
				<div class="form-item">
					<label for="4">Повторите пароль(оставьте поле пустым если не хотите менять пароль)</label>
					<input id="4" name="password_2" minlength="5" maxlength="50" type="password" class="reg-input" placeholder="Повторите ваш пароль...">
				</div>
				<div class="form-item">
					<button name="button-user-update" type="submit" class="submit-btn1 add">Изменить пользователя</button>
				</div>
			</form>
		</div>
	</div>
...
<script>
	...
	let el_get_gender = document.querySelector(".gender option[value='<?=$gender?>']");
	el_get_gender.setAttribute('selected', '');
	let el_get_admin = document.querySelector(".category option[value='<?=$admin?>']");
	el_get_admin.setAttribute('selected', '');
	<? endif; ?>
</script>
